...
(version 2.2.0).  QSim provides instruction-level control of the emulated
environment and detailed information about the executing instruction
stream. ARM64 Linux guests are supported with multiple emulated hardware
...
\begin{itemize}
  ...
  \item{2GB of RAM, at least 4GB of swap and 10GB of disk space}
...
plan to add support for the other types of callbacks in the near future.
...
repository downloads qsim, qemu and the ARM 64-bit filesystem images. It sets up
the required environment variables and builds qsim and qemu as libraries. It
then runs a cache simulation example built using the qsim framework.
...
between the modifications to QEMU and QSim. \end{comment}
...
\begin{verbatim}
    arm-examples/
    arm-examples/simple.cpp
    arm-examples/cachesim.cpp
\end{verbatim}
The ``arm-examples'' are example client program using the QSim library. One of
...
For the purposes of this Chapter, we will examine \texttt{arm-examples/simple.cpp},
...
You can login to the qemu VM using ssh:
  ...
  \item{
    ...
    \begin{itemize}
      ...
      \item{Place the application, \texttt{runme.sh}, and any required data
            files or libraries into a \texttt{.tar} archive and copy it to the
            running ARM qemu instance.}
    \end{itemize}
    }
\end{itemize}
